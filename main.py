...
def ouvrir_page_principale(pseudo, id_user):
    ...
    #ligne 2 : barre de recherche (!!!!!! header_container ici !)
    ...
    #colonne de gauche : top 5 livres
    ...
    #colonne au centre : derniers avis
    ...
    #colonne de droite mes emprunts
...
def retour_inscription():
    #fonction simple pour relancer interface d'inscription
    for widget in root.winfo_children():
        widget.destroy()
...
def valider_inscription():
    ...
    #utilisation de la fonction inscrire_utilisateur dans database.py
    succes, msg = database.inscrire_utilisateur(pseudo, mdp)
    
    if succes:
        messagebox.showinfo("Succès", msg)
        creer_interface_connexion() #redirection vers la connexion
    else:
...
def creer_interface_inscription():
    ...
    #pas de parenthèses après valider_inscription ici
...
def voir_bibliotheque(pseudo_user, id_user):
    ...
    for livre in tous_les_livres:
        ...
        if livre['id_proprietaire'] == id_user:
            ...
            #indicateur visuel pour les livres des autres
